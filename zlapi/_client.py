...
class ZaloAPI(object):
	...
	def changeGroupSetting(self, groupId, defaultMode="default", **kwargs):
		...
		Args:
			...
			**kwargs: Group settings kwargs, Value: (1 = True, 0 = False)
				...
				blockName: Khong cho phep user đoi ten & anh đai dien nhom
				signAdminMsg: Đanh dau tin nhan tu chu/pho nhom
				addMemberOnly: Chi them members (Khi tat link tham gia nhom)
				setTopicOnly: Cho phep members ghim (tin nhan, ghi chu, binh chon)
				enableMsgHistory: Cho phep new members đoc tin nhan gan nhat
				lockCreatePost: Khong cho phep members tao ghi chu, nhac hen
				lockCreatePoll: Khong cho phep members tao binh chon
				...
				lockSendMsg: Khong cho phep members gui tin nhan
				lockViewMember: Khong cho phep members xem thanh vien nhom
				blocked_members: Danh sach members bi chan
	...
	def newlink(self, grid):
			...
			if data.get("error_code") == 0:
				...
				if new_link_info and new_link_info.get("link"):
					...
					return {"success": False, "error_code": 1337, "error_message": "Khong tim thay lien ket moi."}
			else:
				error_code = data.get("error_code")
				error_message = data.get("error_message", "Loi khong xac đinh tu API.")
	...
	def dislink(self, grid):
			...
			if data.get("error_code") == 0:
				return {"success": True, "message": "Đa vo hieu hoa lien ket nhom thanh cong."}
			else:
				error_code = data.get("error_code")
				error_message = data.get("error_message", "Loi khong xac đinh tu API.")
				return {"success": False, "error_code": error_code, "error_message": error_message}
	...
	def remove_blocked_member(self, grid, member_ids):
		...
		if data.get("error_code") == 0:
			return {"success": True, "message": "Xoa thanh vien bi chan thanh cong."}
		else:
			error_code = data.get("error_code")
			error_message = data.get("error_message", "Loi khong xac đinh tu API.")
			return {"success": False, "error_code": error_code, "error_message": error_message}
	...
	def checkGroup(self, link):
		...
		if data.get("error_code") == 0:
			...
			if decoded_data and decoded_data.get("data"):
				...
				return {"error_code": 1337, "error_message": "Khong tim thay du lieu nhom."}
		else:
			error_code = data.get("error_code")
			error_message = data.get("error_message", "Loi khong xac đinh tu API.")
	...
	def sendToDo(self, message_object, content, assignees, thread_id, thread_type, due_date=-1, description=""):
		"""Gui mot todo đen nguoi dung hoac nhom.
		
		Args:
			message_object (Message): Message object goc
			content (str): Noi dung todo
			assignees (list): Danh sach ID nguoi nhan
			thread_id (str): ID cua thread 
			thread_type (ThreadType): Loai thread (USER/GROUP)
			due_date (int): Thoi han (-1 neu khong co)
			description (str): Mo ta (co the trong)
			
		Returns:
			object: Ket qua tu API
