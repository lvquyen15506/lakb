...
def handle_menu_commands(message, message_object, thread_id, thread_type, author_id, bot):
    command_names = "".join([
        ...
        f"➜ ⛅ Thoi tiet ({bot.prefix}weather)\n"
        f"➜ 🤭 Tha thinh ({bot.prefix}love)\n"
        f"➜ 🗯 Chat AI ({bot.prefix}chat)\n"
        f"➜ 🩴 Zet Lo O A ({bot.prefix}zl)\n"
        f"➜ 🎮 Game ({bot.prefix}bc {bot.prefix}tx)\n"
        f"➜ 🍼 Tinh nang khac ({bot.prefix}or)\n"
        f"➜ 🙀 reg hotmail ({bot.prefix}reghotmail)\n"
        f"➜ 💌 Share code ({bot.prefix}share)\n"
        f"➜ 👩‍💼 anh gai ({bot.prefix}girl)\n"
        f"➜ 🤜 xem info ff ({bot.prefix}ff)\n"
        f"➜ 😹 xem đo ff ({bot.prefix}doff)\n"
        f"➜ 🤖 Quan ly Bot ({bot.prefix}mybot)\n"
        f"➜ 🗞️ Tin tuc ({bot.prefix}news)\n"
        f"➜ 🧨 Spam ({bot.prefix}war)\n"
        f"➜ 💬 Noi dung ({bot.prefix}text)\n"
        f"➜ 📀 Video gai ({bot.prefix}vdgirl)\n"
        f"➜ ☁️ Soundcloud ({bot.prefix}scl)\n"
        f"➜ 🐳 Tinh nang an ({bot.prefix}hiden)\n"
        f"➜ 👊 tan cong web ({bot.prefix}attack)\n"
        f"➜ 📔 luu link vd vao file ({bot.prefix}save)\n"
        f"➜ 💍 spam ket ban ff ({bot.prefix}spamff)\n"
        f"➜ 🙀 tao stk  ({bot.prefix}stkxp / {bot.prefix}stk)\n"
        f"➜ 👁 tao qr chuyen khoan ({bot.prefix}qrbank)\n"
        f"➜ 🧠 spam ngl  ({bot.prefix}ngl)\n"
        f"➜ 📣 xem thong tin nhom   ({bot.prefix}group)\n"
        f"➜ 🔊 chuyen van ban thanh giong noi({bot.prefix}voice)\n"
        f"➜ 🇻🇳 xem thông tin ({bot.prefix}detail)\n"
        f"➜ 😞 tạo qr code ({bot.prefix}qrcode)\n"
        f"➜ 🥰 quảng cáo nội dung ({bot.prefix}rao)\n"
        f"➜ 😎 uptime card({bot.prefix}cardinfo)\n"
        f"➜ 📸 tạo ảnh bằng ai logo({bot.prefix}img)\n"
        f"➜ 🥰 spam số điện thoại ({bot.prefix}sms)\n"
