...
def handle_ip_commands(message, message_object, thread_id, thread_type, author_id, bot):
    command_names = "".join([
        f"{get_user_name_by_id(bot, author_id)}\n"
        f"➜ ♨️ IP PC : kkaycraft.fun         \n"
        f"➜ 📢 IP PE : kkaycraft.fun:25578     \n"
    ])
    ...
    bot.sendReaction(message_object, random.choice(reaction), thread_id, thread_type=thread_type)
    bot.sendLocalImage(
...
def generate_menu_image(self, author_id, thread_id, thread_type):
    ...
    try:
        ...
        print(f"✅ Ảnh ip đã được lưu: {OUTPUT_IMAGE_PATH}")
        return OUTPUT_IMAGE_PATH

    except Exception as e:
        print(f"❌ Lỗi xử lý ảnh ip: {e}")
        return None