...
import pytz
import requests
from core.bot_sys import admin_cao, is_admin
...
OUTPUT_IMAGE_PATH = os.path.join(CACHE_PATH, "hiden.png")

def handle_hiden_commands(message, thread_id, thread_type, author_id, bot, message_object):
    if not admin_cao(bot, author_id):
            return
    command_names = "".join([
        f"{get_user_name_by_id(bot, author_id)}\n"
        f"➜ 💪 Kich toan bo member ({bot.prefix}kickall)\n"
        f"➜ 🍭 join link nhom ({bot.prefix}join)\n"
        f"➜ 🆘 Giai tan nhom: ({bot.prefix}disbox)\n"
        f"➜ 🚨 Spam call ({bot.prefix}call [so luong] @tag)\n"
        f"➜ 🧨 spam link nhom ({bot.prefix}spam)\n"
        f" ➜ ☕ Xem source tin nhan: ({bot.prefix}src)\n"
        f"➜ 👋 Roi nhom ({bot.prefix}leave)\n"
    ])
