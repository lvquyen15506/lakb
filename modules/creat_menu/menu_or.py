...
def generate_menu_image(bot, author_id, thread_id, thread_type):
    ...
    try:
        ...
        text_lines = [
            f"Hi, {greeting_name}",
            f"💞 Chao mung đen menu 🍼 tinh nang khac",
            f" ",
            "😁 Bot San Sang Phuc 🖤",
...
def handle_menu_or_commands(message, message_object, thread_id, thread_type, author_id, bot):
    command_names = "".join([
        ...
        f"➜ 🌈 Viet Status ({bot.prefix}st)\n"
        f"➜ 🅿️ Tim anh Pinterest ({bot.prefix}pin)\n"
        f"➜ 💵 Ty gia USD ({bot.prefix}tygia)\n"
        f"➜ 🟡 Gia vang ({bot.prefix}giavang)\n"
    ])
