...
def generate_menu_image(bot, author_id, thread_id, thread_type):
    ...
    try:
        ...
        text_lines = [
            f"Hi, {greeting_name}",
            f"💞 Chao mung đen menu 🚗 phat nguoi",
            f"{bot.prefix}phatnguoi on/off: 🚀 Bat/Tat tinh nang",
            "😁 Bot San Sang Phuc 🖤",
...
def handle_phatnguoi_on(bot, thread_id):
    ...
    return f"🚦Lenh {bot.prefix}phatnguoi đa đuoc Bat 🚀 trong nhom nay ✅"
...
def handle_phatnguoi_off(bot, thread_id):
    ...
    if "phatnguoi" in settings and thread_id in settings["phatnguoi"]:
        ...
        write_settings(bot.uid, settings)
        return f"🚦Lenh {bot.prefix}phatnguoi đa Tat ⭕️ trong nhom nay ✅"
    return "🚦Nhom chua co thong tin cau hinh phatnguoi đe ⭕️ Tat 🤗"
    ...
    def send_phatnguoi_response():
        try:
            ...
            if len(parts) < 2:
                response = (
                    f"🚦{user_name}  ⚙️\n"
                    f"➜ {bot_prefix} [bien so xe]: Tra cuu thong tin phat nguoi.\n"
                    "📌 Bot se tu nhan dien loai xe (o to, xe tai, xe may, xe may đien).\n"
                    f"VD: {bot_prefix} 60K-36752\n"
                    "🤖 BOT luon san sang phuc vu ban! 🌸"
                )
            ...
            for vehicle_code, vehicle_name in VEHICLE_TYPES.items():
                ...
                try:
                    ...
                    if response.status_code == 302:  # Neu server tra ve chuyen huong
                        print(f"⚠️ Chuyen huong bi chan cho URL: {url}")
                        continue

                    if response.status_code != 200:
                        print(f"⚠️ Loi HTTP {response.status_code} khi truy cap URL: {url}")
                        continue

                    response.encoding = "utf-8"
                    soup = BeautifulSoup(response.text, "html.parser")

                    no_violation_message = soup.find("h3", text="Khong tim thay vi pham phat nguoi!")
                    ...
                    for table in tables:
                        ...
                        if resolution_places:
                            violation_info["Noi giai quyet vu viec"] = " | ".join(resolution_places)

                        if violation_info:
                            violations.append(violation_info)
                    break
                except requests.exceptions.RequestException as e:
                    print(f"❌ Loi khi gui yeu cau: {e}")
                    continue
            ...
            if found_vehicle and violations:
                for idx, violation in enumerate(violations, 1):
                    ...
                    bot.replyMessage(
                        Message(text=f"🔹 Vi pham {idx}:\n{violation_details}"),
                        message_object, thread_id=thread_id, thread_type=thread_type, ttl=100000
                    )
            elif not found_vehicle:
                bot.replyMessage(
                    Message(text=f"✅ {user_name}, khong tim thay vi pham nao cho bien so {plate_number}."),
                    message_object, thread_id=thread_id, thread_type=thread_type, ttl=100000
                )
            else:
                bot.replyMessage(
                    Message(text=f"✅ {user_name}, khong tim thay vi pham nao cho bien so {plate_number} ({found_vehicle})."),
                    message_object, thread_id=thread_id, thread_type=thread_type, ttl=100000
                )
