...
def generate_menu_image(bot, author_id, thread_id, thread_type):
    ...
    try:
        ...
        text_lines = [
            ...
            f"{bot.prefix}giavang on/off: 🚀 Bat/Tat tinh nang ",
            "😁 Bot San Sang Phuc 🖤",
...
def get_gold_price_vn():
    ...
    if data["success"]:
        ...
        result = [f"🚦 Cap nhat luc: {latest_date}"]
        ...
        for province, gold_list in provinces.items():
            ...
            for gold_type, buy_price, sell_price in gold_list:
                result.append(f"➜ 🟡 {gold_type}")
                result.append(f"   🛒 Mua vao: {buy_price}")
                result.append(f"   💰 Ban ra: {sell_price}")

        return "\n".join(result)
    else:
        return "❌ Khong the lay du lieu gia vang."
...
def handle_giavang_on(bot, thread_id):
    ...
    return f"🚦Lenh {bot.prefix}giavang đa đuoc Bat 🚀 trong nhom nay ✅"
...
def handle_giavang_off(bot, thread_id):
    ...
    if "giavang" in settings and thread_id in settings["giavang"]:
        ...
        write_settings(bot.uid, settings)
        return f"🚦Lenh {bot.prefix}giavang đa Tat ⭕️ trong nhom nay ✅"
    return "🚦Nhom chua co thong tin cau hinh giavang đe ⭕️ Tat 🤗"
...
def handle_gia_vang_command(bot, message_object, author_id, thread_id, thread_type, command):
    def gia_vang():
        ...
        try:
            ...
            if len(parts) == 1:
                response = (
                    "🎉 Chao mung đen voi menu Gia Vang! ⚙️\n"
                    f"   ➜ {bot.prefix}giavang [vn]: xem gia vang Viet Nam 💰\n"
                )
            else:
