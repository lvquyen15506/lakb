    ...
    try:
        ...
        with open(save_path, 'wb') as file:
            for chunk in response.iter_content(1024):
                file.write(chunk)
        logger.info(f"Anh đa đuoc tai xuong: {save_path}")
        return True
    except requests.exceptions.RequestException as e:
...
def send_search_list(keyword, count, message_object, thread_id, thread_type, client, author_id):
    logger.info(f"Đang lay {count} anh tu Pinterest cho tu khoa: {keyword}")
    ...
    try:
        ...
        logger.info("Đa gui tat ca anh thanh cong.")
    except Exception as e:
    ...
    for image_path in image_paths:
        if os.path.exists(image_path):
            ...
            logger.info(f"Đa xoa anh tam: {image_path}")
...
def handle_pin_on(bot, thread_id):
    ...
    return f"🚦Lenh {bot.prefix}pin đa đuoc Bat 🚀 trong nhom nay ✅"
...
def handle_pin_off(bot, thread_id):
    ...
    if "pin" in settings and thread_id in settings["pin"]:
        ...
        write_settings(bot.uid, settings)
        return f"🚦Lenh {bot.prefix}pin đa Tat ⭕️ trong nhom nay ✅"
    return "🚦Nhom chua co thong tin cau hinh pin đe ⭕️ Tat 🤗"
...
def handle_pro_pin(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if len(content) < 2:
        response = "".join([
            ...
            f"➜ {client.prefix}pin [noi dung tim]: 🔎 Tim anh tu Pinterest\n"
            f"💞 Vi du: {client.prefix}pin meme con meo cuoi hehe"
        ])
